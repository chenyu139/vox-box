...
@router.post("/v1/audio/speech_instruct_stream")
async def speech_instruct_stream(request: Request):
    try:
        form = await request.form()
        keys = form.keys()
        ...
        try:
            ...
            def generate_audio_stream():
                # 调用模型的流式方法
                for audio_chunk in model_instance.speech_instruct_stream(
                    input_text,
                    instruct_text,
                    speech,
                    speed,
            ...
            return StreamingResponse(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=speech_instruct.{response_format}"
                },
            )
        ...
        return HTTPException(
            status_code=500, detail=f"Failed to generate speech_instruct_stream, {e}"
        )
